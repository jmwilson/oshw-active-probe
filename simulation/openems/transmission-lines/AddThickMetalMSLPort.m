function [CSX,port] = AddThickMetalMSLPort( CSX, prio, portnr, materialname, start, stop, dir, evec, varargin )
% [CSX,port] = AddThickMetalMSLPort( CSX, prio, portnr, materialname, start, stop, dir, evec, varargin )
%
...
% 'Thickness'   Thickness of microstrip metal. Metalization is added away from the reference plane.
% 
...
for n=1:2:numel(varargin)
    ...
    elseif (strcmpi(varargin{n},'Thickness'))
        thickness = varargin{n+1};
        if (numel(thickness)>1)
            error 'Thickness must be a scalar value'
        end
    else
...
% direction of height
if start(idx_height)-stop(idx_height) > 0
    height_direction = +1;
else
    height_direction = -1;
end
...
MSL_start = start;
MSL_stop = stop;
MSL_stop(idx_height) = MSL_start(idx_height) + height_direction * thickness;
